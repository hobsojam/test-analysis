...
@main.command()
@click.option("--coverage", "coverage_path", type=click.Path(exists=True), help="Path to Cobertura XML")
@click.option("--stryker", "stryker_path", type=click.Path(exists=True), help="Path to Stryker JSON")
@click.option("--pit", "pit_path", type=click.Path(exists=True), help="Path to PIT mutations.xml")
@click.option("--mutmut", "mutmut_path", type=click.Path(exists=True), help="Path to mutmut junit.xml")
...
def analyze(coverage_path, stryker_path, pit_path, mutmut_path, format, fail_under):
    ...
    if not report.files:
        click.echo("Error: No data found in reports.", err=True)
        sys.exit(1)
