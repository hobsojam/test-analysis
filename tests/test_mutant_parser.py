...
def test_looks_like_mutation_result_new_format():
    assert _looks_like_mutation_result({"mutation_result": {}, "criteria_result": {}}) is True

def test_looks_like_mutation_result_requires_both_dicts():
    assert _looks_like_mutation_result({"mutation_result": "not a dict", "criteria_result": {}}) is False
...
def test_line_from_identification_mutation_identification_key():
    assert _line_from_identification({"mutation_identification": "lib/foo.rb:15:Add"}) == 15
...
def test_line_from_identification_no_match():
    assert _line_from_identification({"mutation_identification": "no-colons-here"}) is None
...
def test_first_int_uses_mutation_identification():
    assert _first_int([{"mutation_identification": "lib/foo.rb:33:Add"}], "missing_key") == 33
...
def test_normalize_status_unknown_passes_through():
    assert _normalize_status("SomeUnknownStatus") == "SomeUnknownStatus"
...
def test_subject_description_falls_back_to_identification():
    assert _subject_description({"identification": "MyClass#method"}, ()) == "MyClass#method"
...
def test_parse_mutant_criteria_result_survived(tmp_path):
    path = _write_session(tmp_path, [{
        "mutation_result": {"source_path": "lib/calc.rb", "mutation_identification": "lib/calc.rb:10:Add"},
